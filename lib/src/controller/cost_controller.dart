...
class CostController extends GetxController {
  ...
  RxList<DailyCost> costContentList = <DailyCost>[].obs;
  ...
  // 월별 소비 내역 가져오기
  void getMonthCostContent(DateTime date) async {
  ...
  void insertCostContent(AssetContent assetContent) async {
    ...
    await db.insert("daily_cost", assetContent.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
    getMonthCostContent(_utilController.date);
    Get.back();
  }
}
